...
- (void)viewDidLoad {
    ...
    // gets access token
    NSError *error;
    NSString *url_string = [NSString stringWithFormat: @"http://weave-sg.herokuapp.com/token/phone"];
...
//    self.videoPlayerView = [[AVPlayerView alloc] initWithFrame:[[UIScreen mainScreen] bounds]];
    ...
    self.remoteView.frame = CGRectMake(650, 220, 150, 150);
}
...
- (void)startPreview {
    ...
    self.camera = [[TVICameraCapturer alloc] initWithSource:TVICameraCaptureSourceBackCameraWide delegate:self];
...
//    playerView.layer.zPosition=1.0;
...
- (void)setupRemoteView {
    ...
    [self.view insertSubview:remoteView aboveSubview:self.previewView];
...
- (void)showInterfaceState:(ViewControllerState)state {
    self.viewPerson.hidden = state != ViewControllerStateRoom;
...
- (void)room:(TVIRoom *)room participantDidDisconnect:(TVIRemoteParticipant *)participant {
    if (self.remoteParticipant == participant && [participant.identity isEqualToString:@"operator"]) {
        [self cleanupRemoteParticipant];
    }
