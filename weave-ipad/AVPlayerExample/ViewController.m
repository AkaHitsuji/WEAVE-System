...
- (void)viewDidLoad {
    ...
    // gets access token
    NSError *error;
    NSString *url_string = [NSString stringWithFormat: @"http://weave-sg.herokuapp.com/token/tablet"];
...
//    self.videoPlayerView = [[AVPlayerView alloc] initWithFrame:[[UIScreen mainScreen] bounds]];
...
- (void)doConnect {
    ...
    [self logMessage:[NSString stringWithFormat:@"Attempting to connect to room %@", roomName]];
}
...
//        self.remoteParticipant = room.remoteParticipants[0];
        NSLog(@"room currently contains: %@",room.remoteParticipants);
        // hardcoded value
        int lenRoom = (int) room.remoteParticipants.count;
        NSLog(@"Length of room is: %d",lenRoom);
        for (int i=0; i<lenRoom; i++) {
            NSLog(@"Checking participant: %@",room.remoteParticipants[i].identity);
            if ([room.remoteParticipants[i].identity isEqualToString:@"phone"]) {
                self.remoteParticipant = room.remoteParticipants[i];
                NSLog(@"%@ participant is being rendered",room.remoteParticipants[i].identity);
            }
        }
...
- (void)room:(TVIRoom *)room participantDidDisconnect:(TVIRemoteParticipant *)participant {
    if (self.remoteParticipant == participant && [participant.identity isEqualToString:@"phone"]) {
        [self cleanupRemoteParticipant];
    }
